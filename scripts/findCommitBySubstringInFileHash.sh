...
# Why: Using the short hashes outputted by `pnpm run debug-info`, this utility can be used to figure out what commit someone is using if they don't have a .git folder based on their last modified file (which is also outputted by `pnpm run debug-info`)
find="$1"
